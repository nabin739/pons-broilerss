...
export class JapaneseQuailComponent {
  products: QuailProduct[] = [
    {
      id: 'JQ001',
      name: 'Japanese Quail',
      description: 'Fresh Japanese quail, perfect for grilling or roasting. Known for its tender meat and delicate flavor.',
      image: 'assets/images/j1.jfif',
      variants: [
        { weight: '1 piece', price: 55 }
      ],
      quantity: 0
    }
  ];
  ...
  addToCart(product: QuailProduct, variant: { weight: string; price: number }) {
    product.quantity = 1;
  ...
  increaseQuantity(product: QuailProduct, variant: { weight: string; price: number }) {
    product.quantity++;
  ...
  decreaseQuantity(product: QuailProduct) {
